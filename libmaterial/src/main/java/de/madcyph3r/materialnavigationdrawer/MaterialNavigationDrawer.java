...
public abstract class MaterialNavigationDrawer<Fragment> extends
		ActionBarActivity implements MaterialSectionOnClickListener,
		MaterialSectionChangeListener, MaterialHeadItem.OnHeadItemDataLoaded,
	...
	public MaterialMenu currentMenu;
	private boolean loadFragmentOnStart;

	// global vars other
	private CharSequence title; // not really needed now, maybe later for
								// restore
	...
	private void initThemeVars(Resources.Theme theme) {
		...
		TypedArray values = theme.obtainStyledAttributes(typedValue.resourceId,
				R.styleable.MaterialAccount);
		titleColor = values.getColor(R.styleable.MaterialAccount_titleColor,
				0x00FFFFFF);
		subTitleColor = values.getColor(
				R.styleable.MaterialAccount_subTitleColor, 0x00FFFFFF);
		headItemBackgroundGradient = values
				.getDrawable(R.styleable.MaterialAccount_backgroundGradient);
	}
	...
	private void initViews() {
		...
		if (drawerHeaderType == DRAWERHEADER_HEADITEMS) {
			...
			headItemBackgroundSwitcher = (ImageView) this
					.findViewById(R.id.background_switcher);
			headItemButtonSwitcher = (ImageButton) this
					.findViewById(R.id.user_switcher);
			headItemBackgroundGradientLL = (LinearLayout) this
					.findViewById(R.id.background_gradient);

			int sdk = android.os.Build.VERSION.SDK_INT;
			if (sdk < android.os.Build.VERSION_CODES.JELLY_BEAN) {
				headItemBackgroundGradientLL
						.setBackgroundDrawable(headItemBackgroundGradient);
			} else {
				headItemBackgroundGradientLL
						.setBackground(headItemBackgroundGradient);
			}
		} else
		...
		if (loadSectionPosition == -1)
			...
			if (currentMenu.getSection(loadSectionPosition) instanceof MaterialSection) {
				...
				if ((currentSection.getTarget() == MaterialSection.TARGET_FRAGMENT)) {
					currentSection.select();
					setFragment((Fragment) currentSection.getTargetFragment(),
							currentSection.getTitle(), null, false);
					changeToolbarColor(currentSection);
				}
			} else {
				throw new RuntimeException(
						"Given Position has not a MaterialSection");
			}
		}
	}
	...
	/*
	 * private void unSelectOldSection(MaterialSection section) { if
	 * (currentSection != null && section != currentSection) {
	...
	private MaterialSection newHeadSection(String title, Drawable icon,
			MaterialMenu menu, int position, boolean refreshMenu) {
		MaterialSection section = new MaterialSection<Fragment>(this, true,
				MaterialSection.TARGET_CLICK, false, this);
	...
	private MaterialSection newHeadSection(String title, Drawable icon,
			MaterialMenu menu, int position) {
		return newHeadSection(title, icon, menu, position, false);
	}

	private MaterialSection newHeadSection(String title, Drawable icon,
			MaterialMenu menu, boolean refreshMenu) {
	...
	public MaterialSection newSection(String title, Drawable icon,
			boolean bottom, MaterialMenu menu, int position, boolean refreshMenu) {
		MaterialSection section = new MaterialSection<Fragment>(this, true,
				MaterialSection.TARGET_CLICK, bottom, this);
		section.setIcon(icon);
		section.setTitle(title);
		// section.setPosition(position);
		menu.addItem(section, position);
	...
	public MaterialSection newSection(String title, Drawable icon,
			boolean bottom, MaterialMenu menu, int position) {
		return newSection(title, icon, bottom, menu, position, false);
	}

	public MaterialSection newSection(String title, Drawable icon,
			boolean bottom, MaterialMenu menu, boolean refreshMenu) {
	...
	public MaterialSection newSection(String title, Drawable icon,
			Fragment target, boolean bottom, MaterialMenu menu, int position,
			boolean refreshMenu) {
		MaterialSection section = new MaterialSection<Fragment>(this, true,
				MaterialSection.TARGET_FRAGMENT, bottom, this);
	...
	public MaterialSection newSection(String title, Drawable icon,
			...
			boolean refreshMenu) {
		return newSection(title, icon, target, bottom, menu, menu.getItems()
				.size(), refreshMenu);
	}
	...
	public MaterialSection newSection(String title, Drawable icon,
			Intent target, boolean bottom, MaterialMenu menu, int position,
			boolean refreshMenu) {
		MaterialSection section = new MaterialSection<Fragment>(this, true,
				MaterialSection.TARGET_ACTIVITY, bottom, this);
		section.setOnClickListener(this);
		section.setIcon(icon);
		section.setTitle(title);
		section.setTarget(target);
	...
	public void onClick(final MaterialSection section, View view) {
		if (section != currentSection) {
			// hide search bar
			if (section.getTitle().equals("Trang chủ")) {
				if (mSearchView != null)
			...
			if (section.getTarget() == MaterialSection.TARGET_FRAGMENT) {
				 ...
				 * this.startActivity(section.getTargetIntent()); finish(); }
				 * else {
				 */
