...
public class ProgressWheel2 extends View {
	private static final String TAG = ProgressWheel2.class.getSimpleName();
	...
	private void parseAttributes(TypedArray a) {
		...
		circleRadius = (int) a.getDimension(
				R.styleable.ProgressWheel2_matProg_circleRadius, circleRadius);

		fillRadius = a.getBoolean(
				R.styleable.ProgressWheel2_matProg_fillRadius, false);

		barWidth = (int) a.getDimension(
				R.styleable.ProgressWheel2_matProg_barWidth, barWidth);

		rimWidth = (int) a.getDimension(
				R.styleable.ProgressWheel2_matProg_rimWidth, rimWidth);

		float baseSpinSpeed = a.getFloat(
				R.styleable.ProgressWheel2_matProg_spinSpeed,
				spinSpeed / 360.0f);
		spinSpeed = baseSpinSpeed * 360;

		barSpinCycleTime = a.getInt(
				R.styleable.ProgressWheel2_matProg_barSpinCycleTime,
				(int) barSpinCycleTime);

		barColor = a.getColor(R.styleable.ProgressWheel2_matProg_barColor,
				barColor);

		rimColor = a.getColor(R.styleable.ProgressWheel2_matProg_rimColor,
				rimColor);

		linearProgress = a.getBoolean(
				R.styleable.ProgressWheel2_matProg_linearProgress, false);

		if (a.getBoolean(
				R.styleable.ProgressWheel2_matProg_progressIndeterminate, false)) {
			spin();
		}
